...
//
// Main message area. Shows:
//  • System messages (isSystem = true) — always visible
//  • Messages from/to the selected peer
//  • Broadcasts
...
class _ChatAreaState extends State<ChatArea> {
  ...
  // Filter messages relevant to the current view.
  // Uses peerId for accurate per-peer filtering — not sender name (which is
  // ambiguous if two peers share a name, and was the cause of "messages going
  // to general chat" on the receiving side).
  List<ChatMessage> _filter(
      List<ChatMessage> all, Peer? peer, ChatGroup? group) {
    if (peer == null && group == null) {
      // No peer selected — show nothing in chat area
    ...
    return all.where((m) {
      ...
      // Messages with an explicit peerId that matches
      if (peer != null && m.peerId == peer.uid) return true;
      // Outgoing messages without peerId — shown in every peer's chat
      // (they were added by chat_screen._handleSend before we could tag them)
  ...
  Widget build(BuildContext context) {
    ...
    final messages = _filter(svc.messages, widget.selectedPeer, widget.selectedGroup);
    final c = widget.colors;

    _maybeScroll(messages.length);

    if (widget.selectedPeer == null && widget.selectedGroup == null) {
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Text(
                  widget.selectedGroup?.name ?? widget.selectedPeer!.name,
                  style: TextStyle(
                    color: c.textPrimary,
