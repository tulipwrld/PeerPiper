...
class SearchTarget {
  final String kind; // peer | group | ai
  final String id;
...
class _SearchDialogState extends State<SearchDialog> {
  ...
  void _doSearch() {
    ...
    final aiHits = service.aiPeerIds
        .map((uid) => service.peers[uid])
        .whereType<Peer>()
        .where((p) => p.name.toLowerCase().contains(q) || p.uid.toLowerCase().contains(q))
        .toList();
    final localAiVisible = service.hasLocalAiHost &&
        (service.myUid != null) &&
        ('local ai'.contains(q) || 'ai'.contains(q) || service.myName.toLowerCase().contains(q));
    ...
    for (final p in aiHits) {
      out.add(_SearchHit(
        target: SearchTarget(kind: 'ai', id: p.uid),
        title: '${p.name} AI',
        subtitle: 'AI chat - ${p.isOnline ? 'online' : 'offline'}',
      ));
    }
    if (localAiVisible) {
      out.add(_SearchHit(
        target: SearchTarget(kind: 'ai', id: service.myUid!),
        title: 'Local AI',
        subtitle: 'AI chat - this device',
      ));
    }
    for (final m in msgHits) {
      final t = m.text.replaceAll('\n', ' ');
      final preview = t.length > 80 ? '${t.substring(0, 80)}...' : t;
      if (m.peerId.startsWith('ai:')) {
        out.add(_SearchHit(
          target: SearchTarget(kind: 'ai', id: m.peerId.substring(3)),
          title: m.sender,
          subtitle: 'AI message - $preview',
        ));
        continue;
      }
      out.add(_SearchHit(
