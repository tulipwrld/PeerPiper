...
class RealP2PService extends P2PService {
  ...
  final Set<String> _aiPeerIds = <String>{};
  ...
  @override String? get myUid => _id?.myId;
  @override String get myIp => _myIp;
  @override bool get hasLocalAiHost => _isLocalAiHost;
  ...
  Future<void> init({String password = 'default'}) async {
    ...
    _isLocalAiHost = await _detectLocalAiHost();
  ...
  @override
  Set<String> get aiPeerIds => Set.unmodifiable(_aiPeerIds);
  ...
  Future<void> sendAiMessage(String targetUid, String text) async {
    if (_pkt == null || _id == null) return;
    if (targetUid == _id!.myId) {
      final answer = await _queryLocalAi(text);
      await DbService.saveMessage(
        peerId: _aiPeerKey(targetUid),
        senderName: 'Me',
        senderId: _id!.myId,
        body: text,
      );
    ...
    final peer = _onlinePeers[targetUid] ?? _knownPeers[targetUid];
    if (peer == null) {
      _addSystem('[AI] Unknown peer: ${_short(targetUid)}');
      return;
    }
    if (!_onlinePeers.containsKey(targetUid)) {
      _addSystem('[AI] Peer is offline: ${peer.name}', peerId: targetUid);
      return;
    }
    final requestId = const Uuid().v4().replaceAll('-', '');
    try {
      final header = await _pkt!.e2eeLlm(peer.xpub, {
        'op': 'request',
      ...
      _addSystem('[AI] Send error: $e', peerId: targetUid);
    }
  }
  ...
  Future<void> _handleIncoming(
    ...
    switch (kind) {
      ...
      case kE2eeLlm:
        ...
        try {
          final key = await CryptoUtils.derivePairwiseKey(
              _id!.xKeyPair, senderXpub, utf8.encode('llm-text-v1'));
          final nonce = CryptoUtils.b64d(header['nonce'] as String);
          final ct = CryptoUtils.b64d(header['ciphertext'] as String);
          final plain =
              await CryptoUtils.aesDecrypt(key, nonce, ct, utf8.encode('llm'));
          ...
          if (op == 'request') {
            ...
            final answer = await _queryLocalAi(text);
            final response = await _pkt!.e2eeLlm(senderXpub, {
              'op': 'response',
              'request_id': requestId,
              'text': answer,
            });
            final replyIps = _onlinePeers[senderId]?.ips ??
                (sourceIp.isNotEmpty ? [sourceIp] : <String>[]);
            if (replyIps.isNotEmpty) {
              await TcpClient.sendPacket(replyIps, response);
            }
          } else if (op == 'response') {
            final text = data['text'] as String? ?? '';
            _addIncoming(
              sender: '$senderName AI',
              text: text,
              peerId: _aiPeerKey(senderId),
            );
            await DbService.saveMessage(
              peerId: _aiPeerKey(senderId),
              senderName: '$senderName AI',
              senderId: senderId,
              body: text,
            );
          }
        } catch (e) {
          _addSystem('[AI] Decrypt/process error: $e', peerId: senderId);
        }
        break;
  ...
  String _aiPeerKey(String peerId) => 'ai:$peerId';

  Future<bool> _detectLocalAiHost() async {
    try {
      final http = HttpClient()..connectionTimeout = const Duration(milliseconds: 700);
      ...
      if (ok) {
        _addSystem('[AI] Local AI endpoint detected on 127.0.0.1:3000');
      }
      return ok;
    } catch (_) {
      return false;
  ...
  Future<String> _queryLocalAi(String prompt) async {
    if (!_isLocalAiHost) {
      return 'AI host is unavailable on this node.';
    }
    try {
      final http = HttpClient()..connectionTimeout = const Duration(seconds: 3);
      final req = await http.postUrl(Uri.parse('http://127.0.0.1:3000/api/mdns/llm'));
      req.headers.contentType = ContentType.json;
      req.write(jsonEncode({'message': prompt}));
      ...
      if (decoded is Map<String, dynamic>) {
        final answer = decoded['answer'] ?? decoded['result'] ?? decoded['text'];
      ...
      return body.trim().isEmpty ? 'AI returned an empty response.' : body;
    } catch (e) {
